...
#
# Modified copy of esphome/components/mipi_rgb/display.py from ESPHome 2026.7.4.
# Change vs upstream: adds the bounce_buffer_lines option.
...
# Number of scanlines per bounce buffer. Two buffers of this size are allocated
...
def _config_schema(config):
    ...
    # ESP-IDF requires the frame buffer size to be an exact multiple of the bounce buffer size.
    if height % config[CONF_BOUNCE_BUFFER_LINES] != 0:
        raise cv.Invalid(
            f"{CONF_BOUNCE_BUFFER_LINES} ({config[CONF_BOUNCE_BUFFER_LINES]}) must divide "
            f"the display height ({height}) exactly",
            [CONF_BOUNCE_BUFFER_LINES],
        )
